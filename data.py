...
import re
# Re-adding imports for youtube_transcript_api and its proxy functionality
from youtube_transcript_api import YouTubeTranscriptApi, NoTranscriptFound, TranscriptsDisabled
from youtube_transcript_api.proxies import WebshareProxyConfig


# --- Configuration ---
# Directory to save the output JSON files (changed to reflect transcript data)
...
def get_video_transcript_segments(video_id):
    ...
    try:
        ...
        transcript_segments = ytt_api.fetch(video_id, languages=['en', 'en-US'])
        ...
        if not transcript_segments:
            ...
            if available_transcripts:
                try:
                    ...
                    transcript_segments = available_transcripts.find_transcript(['a.en', 'a.en-US', 'en', 'en-US']).fetch()
                except Exception:
                    # If specific English not found, try to fetch the first available one
                    if available_transcripts:
                        transcript_segments = available_transcripts[0].fetch()
            
        if not transcript_segments: # If still no segments after all attempts
            print(f"      - No transcript found for video ID: {video_id}.")
            return [] # Return empty list if no transcript
...
def main():
    ...
    all_zoomcamp_metadata = [] # This will store all video metadata
    ...
    for playlist_url in ZOOMCAMP_PLAYLIST_URLS:
        ...
        for i, current_video_id in enumerate(playlist_video_ids):
            ...
            video_metadata = {
                ...
                "transcript_segments": transcript_segments # Add the transcript data here
            }
            all_zoomcamp_metadata.append(video_metadata)

    print("\n--- All Zoomcamp playlists processed. ---")
    print(f"Total unique videos collected: {len(all_zoomcamp_metadata)}")

    # Save all collected metadata into one large JSON file
    output_file_path = os.path.join(OUTPUT_DIR, "all_zoomcamp_metadata_with_transcripts.json")
    try:
